...
func zeroPosition(board *[3][3]int) (Position) {
	for i,_ := range board {
...
func lastMove(s *State) (lastMove string) {
	if len(s.answer) == 0 {
		return
	}
	lastMove = s.answer[len(s.answer)-1]
	return 
}
...
func canMove(s State, direction string) bool {
	...
	case "b":
		if x == 2 || (previousMove == "t") {
			return false
		}
	case "r":
		if y == 2 || (previousMove == "l") {
			return false
		}
	case "l":
...
func swap(board [3][3]int, x1, y1, x2, y2 int) [3][3]int {
	newBoard := board;
...
func calCost(board *[3][3]int) int{
	var cost float64;
	for i:=0;i<3;i++ {
		for j:=0;j<3;j++{
			var x,y int;
...
func Solve(board [3][3]int) (int,[]string,error){
	var minMove int;
	var bestAns []string;
	
	err := isSolvable((*[3][3]int)(&board))
	if err != nil {
		return -1,[]string{},err
	} else {
	...
	for pq.Len() > 0 {
		...
		if topItem.priority + len(topItem.value.answer) < minMove {
			...
			for i:=0;i<4;i++ {
				newState := <- ch
				newCost := calCost((*[3][3]int)(&newState.board))
